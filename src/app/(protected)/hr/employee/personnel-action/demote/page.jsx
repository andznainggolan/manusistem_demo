...
const ACTION = 'Demote'
const ICON   = '⬇️'
const COLOR  = 'from-orange-600 to-amber-600'
...
function Sel({ value, onChange, options, placeholder, disabled = false }) {
  ...
  return (
    <select value={value ?? ''} onChange={onChange} disabled={disabled}
      className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-400 bg-white disabled:bg-gray-50 disabled:text-gray-400'>
...
export default function DemotePage() {
  ...
  const applyToEmployee = useCallback((pa) => {
    updateEmployee(pa.employeeId, { positionId: pa.toPositionId, gradeId: pa.toGradeId })
  ...
  const handleSave = (targetStatus) => {
    ...
    setModal(null)
    flash(targetStatus === 'Applied' ? t('PA Demote berhasil diterapkan.','PA Demote applied successfully.') : t('PA disimpan.','PA saved.'))
  }
    ...
    <div className='min-h-screen bg-gray-50'>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-orange-200 text-sm mt-0.5'>{t('Penurunan posisi atau grade karyawan','Employee position or grade demotion')}</p>
            </div>
          </div>
          <button onClick={() => { setForm({ ...EMPTY, effectiveDate: todayStr() }); setModal({ mode: 'create' }) }}
            className='flex items-center gap-2 bg-white text-orange-700 font-semibold px-5 py-2.5 rounded-xl shadow hover:shadow-md transition-all text-sm'>
            + New PA
          </button>
        </div>
      </div>
      ...
      <div className='px-8 py-4 bg-white border-b flex gap-3 items-center'>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('Cari nama / PA number…', 'Search name / PA number…')}
          className='px-4 py-2 border border-gray-200 rounded-xl text-sm outline-none focus:border-orange-400 w-56' />
        ...
        <div className='bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden'>
          <table className='w-full text-sm'>
            ...
            <tbody className='divide-y divide-gray-50'>
              {myPAs.length === 0 ? (
                <tr><td colSpan={7} className='text-center py-16 text-gray-400'><div className='text-4xl mb-2'>{ICON}</div><p>{t('Belum ada PA Demote','No Demote PA yet')}</p></td></tr>
              ) : myPAs.map(pa => {
                ...
                return (
                  <tr key={pa.id} onClick={() => { setForm({ ...pa }); setModal({ mode: 'view', pa }) }}
                    ...
                    <td className='px-4 py-3'><span className='font-mono text-xs font-semibold text-orange-700 bg-orange-50 px-2 py-0.5 rounded'>{pa.paNumber}</span></td>
                    ...
                    <td className='px-4 py-3 text-xs font-semibold text-orange-700'>{posLabel(pa.toPositionId)}</td>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm'>
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[88vh] flex flex-col overflow-hidden'>
            ...
            <div className='overflow-y-auto flex-1 px-6 py-5 space-y-5'>
              ...
              {form.employeeId && (
                <div>
                  <p className='text-xs font-bold text-gray-400 uppercase tracking-wider mb-3'>{t('TO — Posisi Baru (Lebih Rendah)','TO — New Position (Lower)')}</p>
                  <div className='grid grid-cols-2 gap-4'>
                    ...
                    <Field label={t('New Grade (auto-fill dari posisi)','New Grade (auto-fill from position)')}>
            ...
            <div className='flex items-center justify-between px-6 py-4 border-t bg-gray-50 flex-shrink-0'>
              ...
              {!isView && !isApplied && (
                <div className='flex gap-2'>
                  ...
                  <button onClick={() => handleSave('Applied')} className='px-4 py-2 text-sm font-semibold text-white bg-orange-600 rounded-xl hover:bg-orange-700 shadow'>Apply</button>
                </div>
              )}
              {isView && !isApplied && <button onClick={() => setModal({ mode: 'edit', pa: modal.pa })} className='px-4 py-2 text-sm font-semibold text-white bg-orange-600 rounded-xl hover:bg-orange-700 shadow'>Edit PA</button>}
            </div>
          </div>
        </div>
      )}
