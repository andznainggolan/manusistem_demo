...
const ACTION = 'Transfer Across Company'
const ICON   = '🏢'
const COLOR  = 'from-indigo-700 to-indigo-500'
...
function Sel({ value, onChange, options, placeholder, disabled = false }) {
  ...
  return (
    <select value={value ?? ''} onChange={onChange} disabled={disabled}
      className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-indigo-400 bg-white disabled:bg-gray-50 disabled:text-gray-400'>
...
export default function TransferAcrossCompanyPage() {
  ...
  const handleSave = (targetStatus) => {
    ...
    if (!form.toCompanyId)     return flash(t('Pilih perusahaan tujuan.','Select a target company.'), 'error')
    if (!form.toBusinessUnitId)return flash(t('Pilih Business Unit tujuan.','Select a target Business Unit.'), 'error')
    if (!form.toDepartmentId)  return flash(t('Pilih Department tujuan.','Select a target Department.'), 'error')
    ...
    setModal(null)
    flash(targetStatus === 'Applied' ? t('PA Transfer Across Company berhasil diterapkan.','PA Transfer Across Company applied successfully.') : t('PA disimpan.','PA saved.'))
  }
    ...
    <div className='min-h-screen bg-gray-50'>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-indigo-200 text-sm mt-0.5'>{t('Mutasi lintas perusahaan dalam group','Transfer across companies within the group')}</p>
            </div>
          </div>
          <button onClick={() => { setForm({ ...EMPTY, effectiveDate: todayStr() }); setModal({ mode: 'create' }) }}
            className='flex items-center gap-2 bg-white text-indigo-700 font-semibold px-5 py-2.5 rounded-xl shadow hover:shadow-md transition-all text-sm'>
            + New PA
          </button>
        </div>
      </div>
      ...
      <div className='px-8 py-4 bg-white border-b flex gap-3 items-center'>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('Cari nama / PA number…', 'Search name / PA number…')}
          className='px-4 py-2 border border-gray-200 rounded-xl text-sm outline-none focus:border-indigo-400 w-56' />
          ...
          <table className='w-full text-sm'>
            <thead>
              <tr className='bg-gray-50 border-b'>
                {['PA Number',t('Karyawan','Employee'),t('Dari Company','From Company'),t('Ke Company','To Company'),'Effective Date','Status',''].map(h => (
            ...
            <tbody className='divide-y divide-gray-50'>
              {myPAs.length === 0 ? (
                <tr><td colSpan={7} className='text-center py-16 text-gray-400'><div className='text-4xl mb-2'>{ICON}</div><p>{t('Belum ada PA Transfer Across Company','No Transfer Across Company PA yet')}</p></td></tr>
              ) : myPAs.map(pa => {
                const e = empName(pa.employeeId)
                return (
                  <tr key={pa.id} onClick={() => { setForm({ ...pa }); setModal({ mode: 'view', pa }) }}
                    className='hover:bg-indigo-50/30 cursor-pointer transition-colors'>
                    <td className='px-4 py-3'><span className='font-mono text-xs font-semibold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded'>{pa.paNumber}</span></td>
                    ...
                    <td className='px-4 py-3 text-xs text-gray-500'>{coName(pa.fromCompanyId)}</td>
                    <td className='px-4 py-3 text-xs font-semibold text-indigo-700'>{coName(pa.toCompanyId)}</td>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm'>
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[88vh] flex flex-col overflow-hidden'>
            ...
            <div className='overflow-y-auto flex-1 px-6 py-5 space-y-5'>
              ...
              {form.employeeId && (
                <div className='bg-gray-50 rounded-xl p-4'>
                  ...
                  <div className='grid grid-cols-3 gap-3'>
                    ...
                    <RV label='Division'      value={divName(form.fromDivisionId)} />
                    <RV label='Business Unit' value={buName(form.fromBusinessUnitId)} />
                    <RV label='Department'    value={dpName(form.fromDepartmentId)} />
                    <RV label='Position'      value={posLabel(form.fromPositionId)} />
                    <RV label='Grade'         value={grLabel(form.fromGradeId)} />
                  </div>
                </div>
              )}
              ...
              {form.employeeId && (
                <div>
                  <p className='text-xs font-bold text-gray-400 uppercase tracking-wider mb-3'>{t('TO — Perusahaan & Posisi Tujuan','TO — Target Company & Position')}</p>
                  <div className='grid grid-cols-2 gap-4'>
                    ...
                    <Field label='Division (auto)'>
                      <RV value={divName(form.toDivisionId)} />
                    </Field>
                    ...
                    <Field label='Note'>
                      {isView || isApplied ? <RV value={form.note} />
                        : <textarea value={form.note || ''} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} rows={2}
                            className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-indigo-400 resize-none' />}
                    </Field>
                  </div>
                </div>
              </div>
            ...
            <div className='flex items-center justify-between px-6 py-4 border-t bg-gray-50 flex-shrink-0'>
              ...
              {!isView && !isApplied && (
                <div className='flex gap-2'>
                  ...
                  <button onClick={() => handleSave('Applied')} className='px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow'>Apply</button>
                </div>
              )}
              {isView && !isApplied && <button onClick={() => setModal({ mode: 'edit', pa: modal.pa })} className='px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow'>Edit PA</button>}
            </div>
          </div>
        </div>
      )}
