...
const ICON   = '↔️'
const COLOR  = 'from-blue-700 to-blue-500'
...
function Sel({ value, onChange, options, placeholder, disabled = false }) {
  ...
  return (
    <select value={value ?? ''} onChange={onChange} disabled={disabled}
      className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-400 bg-white disabled:bg-gray-50 disabled:text-gray-400'>
...
export default function TransferPage() {
  ...
  const { positions, grades, companies, businessUnits, departments } = useStructureStore()
  ...
  const handleEmpChange = (empId) => {
    ...
    setForm(f => ({
      ...
      // Transfer: same company, reset BU/dept/pos
      toCompanyId: e.companyId, toDivisionId: e.divisionId,
      toBusinessUnitId: '', toDepartmentId: '', toPositionId: '', toGradeId: '',
  ...
  const applyToEmployee = useCallback((pa) => {
    ...
    addHistory(pa.employeeId, {
      ...
      companyId: pa.fromCompanyId, departmentId: pa.toDepartmentId,
  ...
  const handleSave = (targetStatus) => {
    ...
    setModal(null)
    flash(targetStatus === 'Applied' ? t('PA Transfer berhasil diterapkan.','PA Transfer applied successfully.') : t('PA disimpan.','PA saved.'))
  }
    ...
    <div className='min-h-screen bg-gray-50'>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-blue-200 text-sm mt-0.5'>{t('Mutasi antar departemen dalam satu perusahaan','Transfer between departments within a company')}</p>
            </div>
          </div>
          <button onClick={() => { setForm({ ...EMPTY, effectiveDate: todayStr() }); setModal({ mode: 'create' }) }}
            className='flex items-center gap-2 bg-white text-blue-700 font-semibold px-5 py-2.5 rounded-xl shadow hover:shadow-md transition-all text-sm'>
            + New PA
          </button>
        </div>
      </div>
      ...
      <div className='px-8 py-4 bg-white border-b flex gap-3 items-center'>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('Cari nama / PA number…', 'Search name / PA number…')}
          className='px-4 py-2 border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-400 w-56' />
          ...
          <table className='w-full text-sm'>
            <thead>
              <tr className='bg-gray-50 border-b'>
                {['PA Number',t('Karyawan','Employee'),t('Dari Dept','From Dept'),t('Ke Dept','To Dept'),'Effective Date','Status',''].map(h => (
            ...
            <tbody className='divide-y divide-gray-50'>
              {myPAs.length === 0 ? (
                <tr><td colSpan={7} className='text-center py-16 text-gray-400'><div className='text-4xl mb-2'>{ICON}</div><p>{t('Belum ada PA Transfer','No Transfer PA yet')}</p></td></tr>
              ) : myPAs.map(pa => {
                const e = empName(pa.employeeId)
                return (
                  <tr key={pa.id} onClick={() => { setForm({ ...pa }); setModal({ mode: 'view', pa }) }}
                    className='hover:bg-blue-50/30 cursor-pointer transition-colors'>
                    <td className='px-4 py-3'><span className='font-mono text-xs font-semibold text-blue-700 bg-blue-50 px-2 py-0.5 rounded'>{pa.paNumber}</span></td>
                    ...
                    <td className='px-4 py-3 text-xs text-gray-500'>{dpName(pa.fromDepartmentId)}</td>
                    <td className='px-4 py-3 text-xs font-semibold text-blue-700'>{dpName(pa.toDepartmentId)}</td>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm'>
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[88vh] flex flex-col overflow-hidden'>
            ...
            <div className='overflow-y-auto flex-1 px-6 py-5 space-y-5'>
              ...
              {form.employeeId && (
                <div>
                  <p className='text-xs font-bold text-gray-400 uppercase tracking-wider mb-3'>{t('TO — Departemen Tujuan','TO — Target Department')}</p>
                  <p className='text-xs text-gray-400 mb-3 -mt-2'>Company tetap sama: <strong>{coName(form.toCompanyId)}</strong></p>
                  <div className='grid grid-cols-2 gap-4'>
                    ...
                    <Field label='New Position'>
                      {isView || isApplied ? <RV value={posLabel(form.toPositionId)} />
                        : <Sel value={form.toPositionId} onChange={e => handleToChange('toPositionId', e.target.value)} options={posOpts} placeholder='-- Opsional --' />}
                    </Field>
                    <Field label='New Grade (auto)'>
                      <RV value={form.toGradeId ? grLabel(form.toGradeId) : grLabel(form.fromGradeId) + ' (sama)'} />
                    </Field>
                  </div>
                </div>
              )}
            ...
            <div className='flex items-center justify-between px-6 py-4 border-t bg-gray-50 flex-shrink-0'>
              ...
              {!isView && !isApplied && (
                <div className='flex gap-2'>
                  ...
                  <button onClick={() => handleSave('Applied')} className='px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow'>Apply</button>
                </div>
              )}
              {isView && !isApplied && <button onClick={() => setModal({ mode: 'edit', pa: modal.pa })} className='px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow'>Edit PA</button>}
            </div>
          </div>
        </div>
      )}
