...
const ACTION = 'Extend Contract'
const ICON   = '📅'
const COLOR  = 'from-teal-700 to-teal-500'
...
function Sel({ value, onChange, options, placeholder, disabled = false }) {
  ...
  return (
    <select value={value ?? ''} onChange={onChange} disabled={disabled}
      className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-teal-400 bg-white disabled:bg-gray-50 disabled:text-gray-400'>
...
export default function ExtendContractPage() {
  ...
  // Only Active + Contract/Internship employees
  const empOpts = useMemo(() =>
    employees
      .filter(e => e.status === 'Active' && ['Contract', 'Internship'].includes(e.employmentType))
      .map(e => ({ v: e.id, l: `${e.name} — ${e.nik} (${e.employmentType}, ends: ${e.endDate || '?'})` })),
    [employees])
  ...
  const applyToEmployee = useCallback((pa) => {
    ...
    addHistory(pa.employeeId, {
      ...
      note: `Extend kontrak: ${pa.fromEndDate} → ${pa.toEndDate}. ${pa.note || ''}`.trim(),
    })
  ...
  const handleSave = (targetStatus) => {
    ...
    if (!form.toEndDate)     return flash(t('New End Date wajib diisi.','New End Date is required.'), 'error')
    if (form.toEndDate <= form.fromEndDate) return flash(t('New End Date harus lebih besar dari End Date saat ini.','New End Date must be after the current End Date.'), 'error')
    ...
    setModal(null)
    flash(targetStatus === 'Applied' ? t('Kontrak berhasil diperpanjang.','Contract extended successfully.') : t('PA disimpan.','PA saved.'))
  }
    ...
    <div className='min-h-screen bg-gray-50'>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-teal-200 text-sm mt-0.5'>{t('Perpanjangan kontrak kerja karyawan (Contract / Internship)','Employee contract extension (Contract / Internship)')}</p>
            </div>
          </div>
          <button onClick={() => { setForm({ ...EMPTY, effectiveDate: todayStr() }); setModal({ mode: 'create' }) }}
            className='flex items-center gap-2 bg-white text-teal-700 font-semibold px-5 py-2.5 rounded-xl shadow hover:shadow-md transition-all text-sm'>
            + New PA
          </button>
        </div>
      </div>
      ...
      <div className='px-8 py-4 bg-white border-b flex gap-3 items-center'>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('Cari nama / PA number…', 'Search name / PA number…')}
          className='px-4 py-2 border border-gray-200 rounded-xl text-sm outline-none focus:border-teal-400 w-56' />
          ...
          <table className='w-full text-sm'>
            <thead>
              <tr className='bg-gray-50 border-b'>
                {['PA Number',t('Karyawan','Employee'),'Employment Type',t('End Date Lama','Old End Date'),t('End Date Baru','New End Date'),'Effective Date','Status',''].map(h => (
            ...
            <tbody className='divide-y divide-gray-50'>
              {myPAs.length === 0 ? (
                <tr><td colSpan={8} className='text-center py-16 text-gray-400'><div className='text-4xl mb-2'>{ICON}</div><p>{t('Belum ada PA Extend Contract','No Extend Contract PA yet')}</p></td></tr>
              ) : myPAs.map(pa => {
                ...
                return (
                  <tr key={pa.id} onClick={() => { setForm({ ...pa }); setModal({ mode: 'view', pa }) }}
                    ...
                    <td className='px-4 py-3'><span className='font-mono text-xs font-semibold text-teal-700 bg-teal-50 px-2 py-0.5 rounded'>{pa.paNumber}</span></td>
                    ...
                    <td className='px-4 py-3 text-xs text-gray-600'>{pa.fromEmploymentType}</td>
                    <td className='px-4 py-3 text-xs text-gray-500 line-through'>{pa.fromEndDate || '—'}</td>
                    <td className='px-4 py-3 text-xs font-semibold text-teal-700'>{pa.toEndDate || '—'}</td>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm'>
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[88vh] flex flex-col overflow-hidden'>
            ...
            <div className='overflow-y-auto flex-1 px-6 py-5 space-y-5'>
              <Field label={t('Karyawan * (Contract / Internship aktif)','Employee * (Active Contract / Internship)')}>
              ...
              {form.employeeId && (
                <div className='bg-gray-50 rounded-xl p-4'>
                  <p className='text-xs font-bold text-gray-400 uppercase tracking-wider mb-3'>{t('FROM — Kontrak Saat Ini','FROM — Current Contract')}</p>
                  <div className='grid grid-cols-2 gap-3'>
                    ...
                    <RV label='Current End Date'  value={form.fromEndDate || t('(tidak ada)','(none)')} />
                  </div>
                </div>
              )}
              ...
              {form.employeeId && (
                <div>
                  <p className='text-xs font-bold text-gray-400 uppercase tracking-wider mb-3'>{t('TO — Perpanjangan','TO — Extension')}</p>
                    ...
                    <Field label='New End Date *'>
                      {isView || isApplied ? <RV value={form.toEndDate} />
                        : <input type='date' value={form.toEndDate || ''} onChange={e => setForm(f => ({ ...f, toEndDate: e.target.value }))}
                            ...
                            min={form.fromEndDate} />}
                    </Field>
                    {form.fromEndDate && form.toEndDate && form.toEndDate > form.fromEndDate && (
                      <div className='bg-teal-50 border border-teal-100 rounded-xl px-4 py-2.5 text-xs text-teal-700'>
                        ...
                        {form.fromEndDate && ` (${t('diperpanjang dari','extended from')} ${form.fromEndDate})`}
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className='flex items-center justify-between px-6 py-4 border-t bg-gray-50 flex-shrink-0'>
              ...
              {!isView && !isApplied && (
                <div className='flex gap-2'>
                  ...
                  <button onClick={() => handleSave('Applied')} className='px-4 py-2 text-sm font-semibold text-white bg-teal-600 rounded-xl hover:bg-teal-700 shadow'>Apply</button>
                </div>
              )}
              {isView && !isApplied && <button onClick={() => setModal({ mode: 'edit', pa: modal.pa })} className='px-4 py-2 text-sm font-semibold text-white bg-teal-600 rounded-xl hover:bg-teal-700 shadow'>Edit PA</button>}
            </div>
          </div>
        </div>
      )}
